...
//on prends la langue du 1er admin
	gladys.utils.sql('select language from user where role=\'admin\' order by id').then(function(lang){
		...
		{
				gladys.sentence.create(sentences.sentenceShutdownEn);
				gladys.sentence.create(sentences.sentenceRebootEn);
		}else 
		{
			gladys.sentence.create(sentences.sentenceShutdownFr);
			gladys.sentence.create(sentences.sentenceRebootFr);
		}
	});
